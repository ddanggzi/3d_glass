...
dhd_conf_set_fw_name_by_chip(dhd_pub_t *dhd, char *fw_path, char *nv_path)
{
	...
	if (fw_path[0] == '\0') {
#ifdef BCMDHD_FW_PATH
		printk("BCMDHD_FW_PATH:%s\n",BCMDHD_FW_PATH);
		bcm_strncpy_s(fw_path, MOD_PARAM_PATHLEN-1, BCMDHD_FW_PATH, MOD_PARAM_PATHLEN-1);
		if (fw_path[0] == '\0')
#endif
		{
...
dhd_conf_set_nv_name_by_chip(dhd_pub_t *dhd, char *nv_path)
{
	...
	if (nv_path[0] == '\0') {
#ifdef BCMDHD_NVRAM_PATH
		bcm_strncpy_s(nv_path, MOD_PARAM_PATHLEN-1, BCMDHD_NVRAM_PATH, MOD_PARAM_PATHLEN-1);
		if (nv_path[0] == '\0')
#endif
		{
...
dhd_conf_set_conf_path_by_nv_path(dhd_pub_t *dhd, char *conf_path, char *nv_path)
{
	...
	if (nv_path[0] == '\0') {
#ifdef BCMDHD_NVRAM_PATH
		bcm_strncpy_s(conf_path, MOD_PARAM_PATHLEN-1, BCMDHD_NVRAM_PATH, MOD_PARAM_PATHLEN-1);
		if (nv_path[0] == '\0')
#endif
		{
			printf("nvram path is null\n");
